...
class EdgeDetectionTransform:
    ...
    def sobel(self, tensor):
        ...
        edges = torch.zeros_like(img_tensor)
        ...
        for c in range(channels):
            ...
            edges[:, c:c+1, :, :] = torch.sqrt(edge_x ** 2 + edge_y ** 2)
        edges = edges.squeeze(0)  # Shape: (C, H, W)
        if self.output_image:
            edges = transforms.ToPILImage()(edges)

        return edges

    def canny(self, img_tensor, low_threshold=100, high_threshold=200):
        ...
        if self.output_image:
            edges_tensor = transforms.ToPILImage()(edges_tensor)
